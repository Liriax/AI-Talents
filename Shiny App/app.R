                 ...
                 tabPanel("RESULTS", value = "results",
                          ...
                          fluidRow(
                            ...
                            column(6,
                                   align = "center",
                                   br(),
                                   h5(strong(textOutput("skill_gap")))
                            ),
                            column(3)
                          ),
                          ...
                          fluidRow(
                            ...
                            column(6,
                                   ...
                                   align = "center",
                                   br(),
                                   h5(strong(textOutput("predicted_job")))
...
server <- function(input, output) {
  ...
  getNTopSkillsForJob <- function (title, n){
    df<-read.csv('test.csv',encoding="ISO-8859-1")
    data <- df[ df$job_title == 
                  title, ]
    skills <- scan(text = data[["required_skills"]],
                   what = "character",
                   quote = "",
                   sep = ",")


    skills<-sapply(skills, str_trim)
    skills<-skills[!skills %in% c("development", "management","technician", "analysis", "data", "it", "systems", "software")]
    getFreq <- function (x){
      x = x/nrow(freq_text)
    }

    freq_text <- table(skills)%>% sort(decreasing = T) %>% as.data.frame()
    freq_text$Freq <- sapply(freq_text$Freq,getFreq)
    data <- freq_text[1:n,]
    return(data)
  }
  ...
  output$skillPlot <- renderPlot({
    # job <- findJob()
    job<-input$job
    data <- getNTopSkillsForJob(job, input$skills)
    ggplot(data, aes(skills, Freq))+geom_bar(fill = "indianred2", stat = "identity")+labs(x = "",y="", title = paste("Top skills for", job))+theme_bw()+
